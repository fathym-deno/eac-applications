...
export function establishDenoKvCacheMiddleware(
  ...
  return (async (_req, ctx: EaCApplicationsRuntimeContext) => {
    ...
    const respCacheKey = [
      ...
      ...(ctx.Runtime.URLMatch.Search
        ? ["Search", ctx.Runtime.URLMatch.Search]
        : []),
    ...
    const isCachePathFiltered = !pathFilterRegex ||
      new RegExp(pathFilterRegex, "i").test(reqPath);
