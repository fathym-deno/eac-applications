...
import { AdvancedPreactOptions } from "./preact/AdvancedPreactOptions.ts";
...
import { PreactHookTypes } from "./preact/PreactHookTypes.ts";
...
export class PreactRenderHandler {
  ...
  public async RenderPage(
    ...
    if (this.islandsData.HasData()) {
      const islandsClientPath =
        `./eacIslandsClient.js?revision=${ctx.Runtime.Revision}`;
    ...
    if (Array.from(this.tracking.containers.keys()).length > 0) {
      for (
        ...
        if (children) {
          ...
          pageHtml +=
            `<template id="eac-container-${id}-${target}">${containerHtml}</template>`;
        }
      }
    }
